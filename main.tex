...
%			 given a specific feature set $V$ containing feature vectors from a specific type during time, the attention weights $\alpha_t^{intra}$ and the context vector $c_t^{intra}$ are computed for features $\nu \in V$ based on the typical attention mechanism.
...
%			 given a specific feature set $V = \{\nu^1, \nu^2, \cdots, \nu^n\}$ containing $n$ feature vectors from different modalities, the attention weights $\alpha_t^{inter}$ and the context vector $c_t^{inter}$ are computed for features $\nu \in V$ based on the typical attention mechanism.
		\end{enumerate}
...
%	Figure \ref{fig:attnss} displays the structure of different attention layers proposed by Wu et al. \cite{wu2018hierarchical}. In the low-level attention layer, an intra-modality attention is applied on the temporal features first. Then an inter-modality attention is applied on the motion features and the context vector from the temporal features to select between the temporal and the motion features, which generates $C_t^{mt}$. In parallel, an intra-attention modality is applied on the audio features, which generates the context vector $C_t^{aud}$. Finally, the average vector of the $C_t^{mt}$ and the $C_t^{aud}$ is passed on. In the high-level attention mechanism, an intra-modality attention is used to generate the context vector of the semantic labels. Also, in the sequential attention layer, an intra-modality attention is used to aggregate the context vectors from the low-level and the high-level attention layers.
	
%	The low-level attention layer is designed to extract the context vector from a combination of the temporal features, the motion features, and the audio features. An intra-modality attention is applied on the temporal features first, which generates a context vector $C_t^{tem}$. Then an inter-modality attention is applied on the $C_t^{tem}$ and the motion features, which generates a context vector $C_t^{mt}$. In parallel, an intra-modality attention is applied on the audio features which generates the $C_t^{aud}$. Finally the average vector of the $C_t^{mt}$ and $C_t{aud}$, called $L_t$, is passed as the context vector generated by the low-level attention to the rest of the model.
%		
		\begin{figure}[H]
			\centering
...
%		The results reported in the literature indicate the superior performance of the encoder-decoder architecture in a variety of tasks.
		\\
